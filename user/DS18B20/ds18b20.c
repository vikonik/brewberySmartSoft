...
DS18B20_Status DS18B20_ConvertT(DS18B20 *sensor, DS18B20_WaitCondition waitCondition)
{
  ...
  if (waitCondition == DS18B20_DATA)						//Ждём пока преобразование не закончится, периодически проводя опрос
  {
    WaitForConversionFinished(sensor);
  }

  if (waitCondition == DS18B20_DELAY)						//Выжидаем паузу в соответствии с datasheet
  {
    ...
    switch (sensor->configRegister)
    {
      ...
      default:
		...
		delay_ms(delayValueMs);
  }
	
	if (waitCondition == DS18B20_NONE)						//Запускаем преобразование и не ждём
	{
	}
...
/*----------------------------------------------------------------------------*/
void DS18B20_Init(DS18B20 *sensor)
{	
	sensor->isConnected = 0;
  sensor->uart = OneWire_Init();
