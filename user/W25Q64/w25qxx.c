...
#include "cmsis_os.h"
#else
#define	W25qxx_Delay(delay)		delay_ms(delay)//W25qxx_Spi(W25QXX_DUMMY_BYTE)
#endif

/*
Подменяем HALовские функции
*/
#define HAL_GPIO_WritePin(port, pin,state) PORT_WriteBit(port, pin,state)
...
{
	uint8_t	ret;
	ret = SPI_transmit(_W25QXX_SPI, Data);
	return ret;	
}

void HAL_SPI_Receive(MDR_SSP_TypeDef* spi, uint8_t* pBuffer, uint16_t size, uint16_t a){
	while(size--){
		*pBuffer = SPI_transmit(_W25QXX_SPI, *pBuffer);
		pBuffer++;
	}
}

void HAL_SPI_Transmit(MDR_SSP_TypeDef* spi,uint8_t* pBuffer,uint16_t size,uint16_t a){
	while(size--){
		SPI_transmit(_W25QXX_SPI, *pBuffer++);
	}
}

void W25qxx_SPI_EnableSS_m(MDR_PORT_TypeDef* port,uint16_t pin){
	port->CLRTX |= _W25QXX_CS_PIN; //Справедливо только для Миландра
}

void W25qxx_SPI_DisableSS_m(MDR_PORT_TypeDef* port,uint16_t pin){
	port->SETTX |= _W25QXX_CS_PIN;
}
//###################################################################################################################
uint32_t W25qxx_ReadID(void)
{
  uint32_t Temp = 0, Temp0 = 0, Temp1 = 0, Temp2 = 0;
  W25qxx_SPI_EnableSS_m(PORT_FLASH,_W25QXX_CS_PIN);
  W25qxx_Spi(0x9F);
  Temp0 = W25qxx_Spi(W25QXX_DUMMY_BYTE);
  Temp1 = W25qxx_Spi(W25QXX_DUMMY_BYTE);
  Temp2 = W25qxx_Spi(W25QXX_DUMMY_BYTE);
  W25qxx_SPI_DisableSS_m(PORT_FLASH, _W25QXX_CS_PIN);
...
void W25qxx_ResetChip(void){
		W25qxx_Delay(1);
	W25qxx_SPI_EnableSS_m(PORT_FLASH,_W25QXX_CS_PIN);
	W25qxx_Spi(ENABLE_RESET);
  W25qxx_SPI_DisableSS_m(PORT_FLASH, _W25QXX_CS_PIN);
	
		W25qxx_Delay(1);
	W25qxx_SPI_EnableSS_m(PORT_FLASH,_W25QXX_CS_PIN);
		W25qxx_Spi(RESET_DEVICE);
  W25qxx_SPI_DisableSS_m(PORT_FLASH, _W25QXX_CS_PIN);
}
